...
/**
 * @file    bench_bignum_is_zero.c
 * @brief   Single-thread benchmark for bignum_is_zero.
 */
/* ------------------------------------------------------------------ */
#include <errno.h>
#include <inttypes.h>
...
int main(int argc, char **argv)
{
    ...
    for (uint64_t i = 0; i < options.warmup; ++i) {
        result.len = (options.mode == DATA_ALL_ZERO ||
                           (options.mode == DATA_MIXED && ((i % options.data_count) % 2U) == 0U))
                          ? 0U : options.src_len;
    ...
    for (uint64_t i = 0; i < options.iterations; ++i) {
        ...
        checksum ^= (uint64_t)bignum_is_zero(&result) + (uint64_t)result.len + i;
        ++successful;
    }
    clock_gettime(CLOCK_MONOTONIC, &end);
    elapsed = seconds_between(start, end);

    printf("benchmark=bignum_is_zero_st data_mode=%s seed=%" PRIu64
