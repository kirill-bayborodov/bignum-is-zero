...
/**
 * @file    bench_bignum_is_zero_mt.c
 * @brief   Multithread benchmark for bignum_is_zero.
 */
/* ------------------------------------------------------------------ */
#include <errno.h>
#include <inttypes.h>
...
static void *worker(void *opaque)
{
    ...
    for (uint64_t i = 0; i < total; ++i) {
        size_t row = (i + arg->thread_id) % arg->options->data_count;
        const uint64_t *source = arg->data[row];
        (void)source;
        result.len = (arg->options->mode == DATA_ALL_ZERO ||
                      (arg->options->mode == DATA_MIXED && (row % 2U) == 0U))
                         ? 0U : arg->options->src_len;
        if (bignum_is_zero(&result) == BIGNUM_IS_ZERO_ERROR_NULL_ARG) {
            arg->failed = 1;
            return NULL;
        }
        if (i >= arg->options->warmup) {
            arg->checksum ^= (uint64_t)bignum_is_zero(&result) +
                             (uint64_t)result.len + i;
            ++arg->successful;
        }
    }
    return NULL;
...
int main(int argc, char **argv)
{
    ...
    printf("benchmark=bignum_is_zero_mt data_mode=%s seed=%" PRIu64
